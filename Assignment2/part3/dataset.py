...
class PalindromeDataset(data.Dataset):
    ...
    #     self.half_length = math.ceil(self.seq_length / 2)
    #     max_num = 10 ** self.half_length
    #     self.total_len = total_len  # Cap total_len at max_num

    #     self.data = np.random.default_rng().choice(max_num, self.total_len, replace=True)
    #     self.mapping = np.eye(10) if one_hot else np.arange(10).reshape(10, 1)
    ...
    # def __getitem__(self, idx):
    #     full_palindrome = self.generate_palindrome(self.data[idx])
    #     inputs, labels = full_palindrome[:-1], int(full_palindrome[-1])
    #     inputs = self.mapping[inputs].astype(np.float32)
    #     return inputs, labels

    # def generate_palindrome(self, num):
    #     half = tuple(map(int, str(num)))
    #     left = np.zeros(self.half_length, dtype=int)
    #     left[-len(half):] = half
    #     right = left[::-1]
    #     if self.seq_length % 2 == 0:
    #         return np.concatenate((left, right))
